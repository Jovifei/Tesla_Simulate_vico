...
def render_timbre_map(phase: np.ndarray, rpm: np.ndarray, load: np.ndarray, boost: np.ndarray, throttle: np.ndarray, config: dict, sample_counter: int = 0) -> dict[str, np.ndarray]:
    ...
    harmonic = order_gains[:, 0] * np.sin(shaft) + order_gains[:, 1] * np.sin(2.0 * shaft) + order_gains[:, 2] * np.sin(3.0 * shaft) + order_gains[:, 3] * np.sin(5.0 * shaft)
    sideband = (0.16 + 0.24 * throttle) * order_gains[:, 1] * (np.sin(shaft * (1.0 + 0.015 * rpm_factor) + 0.13 * np.sin(phase * 0.5)) + 0.5 * np.sin(shaft * (1.0 - 0.015 * rpm_factor)))
    index = np.arange(phase.size, dtype=np.float64) + float(sample_counter)
    broadband = order_gains[:, 2, None] * (0.55 * np.sin(index[:, None] * 0.017 + phase[:, None] * 0.31) + 0.35 * np.sin(index[:, None] * 0.041 + phase[:, None] * 0.73) + 0.10 * np.sin(index[:, None] * 0.097))
    casing = order_gains[:, 0, None] * (0.18 + 0.20 * load)[:, None] * (np.sin(phase[:, None] * 4.7 + 0.4) + 0.35 * np.sin(phase[:, None] * 9.3))
    intake_gain = float(unwrap(config, "intake_model"))
    intake = intake_gain * np.sqrt(np.clip(load * (0.25 + throttle), 0.0, 1.5)) * rpm_factor * (0.7 * np.sin(phase * 3.0 + 0.35) + 0.3 * np.sin(phase * 7.0))
