...
def _row() -> dict[str, object]:
    return {
        ...
        "candidate_sha256": "a" * 64, "package_manifest_sha256": "c" * 64, "identity_score": 3, "realism_score": 3, "low_frequency_score": 3,
...
def test_named_feedback_checks_sha_and_never_declares_human_pass() -> None:
    receipt = validate_named_feedback([_row()], {"candidate.wav": "a" * 64}, package_manifest_sha256="c" * 64)
...
def test_duplicate_or_bad_sha_feedback_is_rejected() -> None:
    with pytest.raises(ValueError, match="duplicate"):
        validate_named_feedback([_row(), _row()], {"candidate.wav": "a" * 64}, package_manifest_sha256="c" * 64)
    with pytest.raises(ValueError, match="SHA"):
        validate_named_feedback([_row()], {"candidate.wav": "b" * 64}, package_manifest_sha256="c" * 64)


def test_manifest_sha_mismatch_is_rejected() -> None:
    with pytest.raises(ValueError, match="manifest"):
        validate_named_feedback([_row()], {"candidate.wav": "a" * 64}, package_manifest_sha256="d" * 64)
