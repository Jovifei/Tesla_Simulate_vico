...
class EngineStateInput:
    ...
    def validate(self, sample_rate_hz: int) -> None:
        ...
        if any(
            abs((later - earlier) - period) > 1e-12
            for earlier, later in zip(self.timestamp, self.timestamp[1:])
        ):
...
def build_default_engine_state_cases() -> dict[str, EngineStateInput]:
    ...
    for name, profile in schema["case_profiles"].items():
        ...
        rpm = tuple(
            float(profile["rpm"][0]) + (float(profile["rpm"][1]) - float(profile["rpm"][0])) * item
            for item in fraction
        )
        load = tuple(
            float(profile["load"][0])
            + (float(profile["load"][1]) - float(profile["load"][0])) * item
            for item in fraction
        )
        speed = tuple(item * speed_per_rpm for item in rpm)
        acceleration = ((speed[1] - speed[0]) * rate,) + tuple(
            (now - previous) * rate for previous, now in zip(speed, speed[1:])
        )
        state = EngineStateInput(
            name,
            tuple(index / rate for index in range(count)),
            rpm,
            speed,
            acceleration,
            load,
    ...
    for index, rpm in enumerate(state.rpm):
        ...
        for item in profile["parameters"].values():
            ...
            order_sum += (
                ...
                * balance
                * math.sin(order * phase + float(item["phase_rad"]))
            )
        ...
        desired = (
            ...
            * (
                ...
                + load * _value(parameters, "transient_load_span")
            )
        )
        time_constant = (
            _value(parameters, "attack_s") if desired > transient else _value(parameters, "decay_s")
        )
        ...
        amplitude = _value(parameters, "base_pressure_pa") + load * _value(
            parameters, "load_pressure_span_pa"
        )
        samples.append(
            speed_mode * amplitude * order_sum
            + transient * math.sin(_value(parameters, "transient_carrier_order") * phase)
        )
    return PressureTrace.uniform(
        "engine_excitation.v04:" + state.case_id,
        samples,
        rate,
