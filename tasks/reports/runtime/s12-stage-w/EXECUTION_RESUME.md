...
- Current synthetic evidence roots (v27, authoritative): `bakeoff_final_remediation_v27`, `migration_final_remediation_rx7_v27`, `migration_final_remediation_ferrari_v27`; manifest SHA-256 values are recorded in the compact receipt, artifact manifest and `v27_evidence_binding`. The v24 roots remain the Task5D-era byte record and are not current.
...
## v27 closure (2026-08-29)

- Task 6Z first run was BLOCKED at Gate 1 only (historical `task-6c-green-output.txt` whitespace tripped the Track-P guard); Task 6AA repaired this configuration-only via `.gitattributes` (commit `803c31f`).
