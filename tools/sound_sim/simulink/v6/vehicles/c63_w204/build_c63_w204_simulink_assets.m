%BUILD_C63_W204_SIMULINK_ASSETS Create the isolated C63 model and dictionary.

scriptDir = fileparts(mfilename("fullpath"));
soundRoot = fileparts(fileparts(fileparts(fileparts(scriptDir))));
matlabRoot = fullfile(soundRoot, "matlab");
v6Root = fullfile(matlabRoot, "v6");
addpath(matlabRoot, v6Root);

profile = v6_vehicle_profile("c63_w204");
dictionaryPath = fullfile(scriptDir, "c63_w204_v6_params.sldd");
if isfile(dictionaryPath)
    dictionary = Simulink.data.dictionary.open(dictionaryPath);
else
    dictionary = Simulink.data.dictionary.create(dictionaryPath);
end
designData = getSection(dictionary, "Design Data");
try
    entry = getEntry(designData, "C63_W204_V6");
    setValue(entry, profile);
catch
    addEntry(designData, "C63_W204_V6", profile);
end
saveChanges(dictionary);
close(dictionary);

sourceModel = fullfile(soundRoot, "simulink", "engine_sound_v6.slx");
targetModel = fullfile(scriptDir, "c63_w204_engine_sound_v6.slx");
if bdIsLoaded("c63_w204_engine_sound_v6")
    close_system("c63_w204_engine_sound_v6", 0);
end
load_system(sourceModel);
save_system("engine_sound_v6", targetModel);
set_param("c63_w204_engine_sound_v6", "DataDictionary", ...
    "c63_w204_v6_params.sldd");
save_system("c63_w204_engine_sound_v6");
close_system("c63_w204_engine_sound_v6");
close_system("engine_sound_v6", 0);
fprintf("C63 V6 Simulink assets: %s\n", scriptDir);
