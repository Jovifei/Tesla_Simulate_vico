function outputDir = v6_render_vehicle_artifacts(profile, iterationName, fitReport)
%V6_RENDER_VEHICLE_ARTIFACTS Render one isolated V6 vehicle iteration.
...
outputDir = fullfile(projectRoot, "build", "sound-sim", "matlab-classics-v6", ...
    profile.name, iterationName);
if ~isfolder(outputDir)
    mkdir(outputDir);
end

prefix = profile.name + "_v6";
write_wav(fullfile(outputDir, prefix + "_external_96k.wav"), audio, ...
    profile.audio.sample_rate_hz, 24);
write_wav(fullfile(outputDir, prefix + "_external_48k.wav"), ...
    resample(audio, profile.audio.export_sample_rate_hz, profile.audio.sample_rate_hz), ...
    profile.audio.export_sample_rate_hz, 16);
write_stem(fullfile(outputDir, prefix + "_exhaust.wav"), result.layers.exhaust, profile.audio.sample_rate_hz);
write_stem(fullfile(outputDir, prefix + "_afterfire.wav"), result.layers.afterfire, profile.audio.sample_rate_hz);
write_stem(fullfile(outputDir, prefix + "_mechanical.wav"), result.layers.mechanical, profile.audio.sample_rate_hz);
write_stem(fullfile(outputDir, prefix + "_induction.wav"), result.layers.induction, profile.audio.sample_rate_hz);
write_stem(fullfile(outputDir, prefix + "_rasp.wav"), result.layers.rasp, profile.audio.sample_rate_hz);
write_stem(fullfile(outputDir, prefix + "_cabin.wav"), result.layers.cabin, profile.audio.sample_rate_hz);
write_stem(fullfile(outputDir, prefix + "_speaker.wav"), result.layers.speaker, profile.audio.sample_rate_hz);
...
trace = table(result.time_s(traceIndex).', result.state.rpm(traceIndex).', ...
    ...
    result.layers.induction(traceIndex).', result.layers.rasp(traceIndex).', ...
    result.state.combustion_variation(traceIndex).', ...
    result.state.combustion_timing_jitter_deg(traceIndex).', ...
    VariableNames=["time_s", "rpm", "load", "torque_nm", "spark_deg", ...
    "lambda", "egt_k", "dfco", "sound_speed_mps", "exhaust", "afterfire", ...
    "mechanical", "induction", "rasp", "combustion_variation", ...
    "combustion_timing_jitter_deg"]);
writetable(trace, fullfile(outputDir, prefix + "_trace.csv"));
writetable(result.events, fullfile(outputDir, prefix + "_events.csv"));
...
write_text(fullfile(outputDir, prefix + "_params.json"), jsonencode(payload, PrettyPrint=true));
write_plot(fullfile(outputDir, prefix + "_analysis.png"), result, audio, profile);
