...
engine, optionally consumes the final per-vehicle Stage-AE diagnostic fit, applies
one attenuation-only package gain per vehicle, and writes a fully standalone HTML
A/B page with no remote CSS/JS dependency.
"""
...
from ..event_domain.config_schema import validate_config
...
def _standalone_html(vehicle_name: str, records: list[dict]) -> str:
    ...
    store = {}
    for rec in records:
        cid = f"{rec['scene']}_candidate"
        rid = f"{rec['scene']}_ref"
        store[cid] = rec["candidate_b64"]
        store[rid] = rec["reference_b64"]
        ref_button = (
            f'<button onclick="play(\'{rid}\')">▶ 真车原声 (B)</button>'
            if rec["reference_b64"]
            else '<button disabled>无真车对照</button>'
        )
        cards.append(
            f'<section><h3>{rec["scene"]}</h3><div class="buttons">'
            f'<button onclick="play(\'{cid}\')">▶ 算法声音 (A)</button>{ref_button}</div></section>'
        )
...
def _resolve_fit_config(config_root: Path | None, vehicle_key: str) -> tuple[dict[str, Any] | None, str | None, str | None]:
    """Resolve the final chained diagnostic config without guessing from timestamps."""
    if config_root is None:
        return None, None, None
    vehicle_root = config_root / vehicle_key
    candidates = [
        vehicle_root / "final_r3_diagnostic_fit.json",
        vehicle_root / "afterfire" / "final_r3_diagnostic_fit.json",
        vehicle_root / "induction" / "final_r3_diagnostic_fit.json",
        vehicle_root / "path" / "final_r3_diagnostic_fit.json",
        vehicle_root / "body" / "final_r3_diagnostic_fit.json",
    ]
    for path in candidates:
        if not path.is_file():
            continue
        payload = validate_config(json.loads(path.read_text(encoding="utf-8")))
        expected = VEHICLES[vehicle_key].config_id
        if payload["vehicle_id"] != expected:
            raise ValueError(f"fit config {path} has vehicle_id={payload['vehicle_id']!r}; expected {expected!r}")
        digest = hashlib.sha256(path.read_bytes()).hexdigest()
        return payload, str(path), digest
    return None, None, None
...
def render_vehicle(
    ...
    config_override, config_source, config_sha256 = _resolve_fit_config(config_root, vehicle_key)
    renderer = CanonicalStageAERenderer(
        profile.config_id,
        random_seed=seed,
        ir_spec=ir_spec,
    ...
    for index, scene in enumerate(SCENES, 1):
        candidate = out / f"{index:02d}_{scene}.wav"
        _write_wav(candidate, monitor[scene])
        files.append(
            {
                "role": "candidate",
                "scene": scene,
        ...
        if reference_root:
            for candidate_ref in (
                reference_root / vehicle_key / f"ref_{scene}.wav",
                reference_root / vehicle_key / "web_audio" / f"ref_{scene}.wav",
            ):
        ...
        records.append(
            {
                "scene": scene,
                "candidate_b64": _wav_bytes(candidate),
                "reference_b64": _wav_bytes(ref) if ref else "",
            }
        )
    (out / "index_standalone.html").write_text(_standalone_html(profile.display_name, records), encoding="utf-8")
    manifest = {
        "schema": "s12.stage_ae.audition.v1",
        "vehicle": vehicle_key,
        "config_id": profile.config_id,
        "config_source": config_source or "repository_default",
        "config_sha256": config_sha256,
        "random_seed": seed,
        "package_gain_db": gain_db,
        "gain_policy": "ONE_ATTENUATION_ONLY_GAIN_FOR_ALL_SCENES",
        "canonical_renderer": "PersistentEventDomainEngine",
        "ir_asset": ir_spec.asset_id if ir_spec else None,
        "ir_sha256": ir_spec.sha256 if ir_spec else None,
        "ir_rights_status": ir_spec.rights_status if ir_spec else None,
        "files": files,
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--config-root", type=Path, help="Optional Stage-AE fit root containing <vehicle>/<family>/final_r3_diagnostic_fit.json")
    parser.add_argument("--ir-manifest", type=Path)
    parser.add_argument("--seed", type=int, default=20260905)
    args = parser.parse_args(argv)
    keys = list(VEHICLES) if args.vehicle == "all" else [args.vehicle]
    for key in keys:
        render_vehicle(key, args.output_root, args.reference_root, args.ir_manifest, args.seed, args.config_root)
    return 0
