...
def _parameter_values() -> tuple[dict[str, float], tuple[tuple[float, float, float], ...]]:
    ...
    orders = tuple(
        (float(entry["order"]), float(entry["amplitude"]), float(entry["phase_rad"]))
    ...
    def accept(
        ...
        if previous is not None:
            ...
            valid = (
                valid
                and elapsed_s > 0.0
                and abs(candidate.rpm - previous.rpm) <= self.max_rpm_per_s * elapsed_s
            )
        if valid:
        ...
        if previous is None:
            timestamp_s = (
                candidate.timestamp_s
                if math.isfinite(candidate.timestamp_s) and candidate.timestamp_s >= 0.0
                else 0.0
            )
            return VehicleState.synthetic_idle(timestamp_s), True
        timestamp_s = (
            candidate.timestamp_s
            if math.isfinite(candidate.timestamp_s) and candidate.timestamp_s > previous.timestamp_s
            else previous.timestamp_s + fallback_interval_s
        )
...
class _RuntimeExcitation:
    ...
    def render_block(
        self, previous: VehicleState, current: VehicleState, frame_count: int = BLOCK_SAMPLES
    ) -> list[float]:
        samples = []
        ...
        for index in range(frame_count):
            ...
            target_load = (
                previous.load
                + (current.load - previous.load) * fraction
                + previous.throttle
                + (current.throttle - previous.throttle) * fraction
            ) * 0.5
            self.smoothed_rpm = (
                target_rpm
                if self.smoothed_rpm is None
                else self.smoothed_rpm + (target_rpm - self.smoothed_rpm) * smoothing
            )
            self.smoothed_load = (
                target_load
                if self.smoothed_load is None
                else self.smoothed_load + (target_load - self.smoothed_load) * smoothing
            )
            operating_point = self.library.evaluate(self.smoothed_rpm, self.smoothed_load)
            self.phase = math.fmod(
                self.phase + 2.0 * math.pi * self.smoothed_rpm / 60.0 / self.sample_rate_hz,
                2.0 * math.pi,
            )
            order_sum = 0.0
            for order, amplitude, phase_rad in self.orders:
                ...
                balance = (
                    1.0 + high_order * self.smoothed_load * self.parameters["high_order_load_gain"]
                )
                order_sum += (
                    amplitude
                    * operating_point.excitation_gain
                    * harmonic_scale
                    * balance
                    * math.sin(order * self.phase + phase_rad)
                )
            rpm_rate = (
                0.0
                if self.previous_rpm is None
                else (self.smoothed_rpm - self.previous_rpm) * self.sample_rate_hz
            )
            acceleration = (
                previous.acceleration_mps2
                + (current.acceleration_mps2 - previous.acceleration_mps2) * fraction
            )
            target_transient = (
                self.parameters["transient_gain"]
                * operating_point.transient_gain
                * (
                    abs(acceleration) / self.parameters["acceleration_reference_mps2"]
                    + abs(rpm_rate) / self.parameters["rpm_rate_reference"]
                )
                * (
                    self.parameters["transient_load_floor"]
                    + self.smoothed_load * self.parameters["transient_load_span"]
                )
            )
            time_constant = (
                self.parameters["attack_s"]
                if target_transient > self.transient
                else self.parameters["decay_s"]
            )
            self.transient += (target_transient - self.transient) / max(
                1.0, time_constant * self.sample_rate_hz
            )
            speed = previous.speed_mps + (current.speed_mps - previous.speed_mps) * fraction
            speed_mode = self.parameters["speed_mode_floor"] + self.parameters[
                "speed_mode_span"
            ] * min(1.0, speed / self.parameters["speed_cruise_mps"])
            samples.append(
                speed_mode * order_sum
                + self.transient * math.sin(self.parameters["transient_carrier_order"] * self.phase)
            )
...
class EngineSoundRuntime:
    ...
    def _accept_state(
        ...
        accepted, fallback = self._sanitizer.accept(
            state, self._previous_ingested_state, fallback_interval_s
        )
    ...
    def _render(
        self,
        segments: tuple[tuple[VehicleState, VehicleState, int], ...],
        fallback: bool,
    ...
    def audio_callback(self, state: VehicleState | None = None) -> PCMFrame:
        ...
        return self._render(
            ...
            second_transition
            if second_transition.progress >= first_transition.progress
            else first_transition,
        )
