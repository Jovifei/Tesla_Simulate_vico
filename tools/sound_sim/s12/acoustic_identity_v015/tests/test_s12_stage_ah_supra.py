...
def test_supra_uses_fixed_parent_denominator(fake_ir):
    rpm, throttle = _curves()
    anchor = SupraEngine(output_policy="legacy_clip_v1", ir=fake_ir, scene_ids=("scene_a",))
    anchor.render_track(rpm, throttle, 1.0)
    candidate = SupraEngine(output_policy="linked_soft_ceiling_v1", parent_peaks=anchor.parent_peaks, ir=fake_ir, scene_ids=("scene_a",))
...
def test_supra_parent_peak_is_bound_to_scene_and_trace(fake_ir):
    rpm, throttle = _curves()
    engine = SupraEngine(output_policy="legacy_clip_v1", ir=fake_ir, scene_ids=("scene_a",))
...
def test_supra_rejects_parent_peak_for_different_scene(fake_ir):
    rpm, throttle = _curves()
    anchor = SupraEngine(output_policy="legacy_clip_v1", ir=fake_ir, scene_ids=("scene_a",))
    anchor.render_track(rpm, throttle, 1.0)
    candidate = SupraEngine(
        output_policy="linked_soft_ceiling_v1",
...
def test_supra_unverified_reference_is_not_promotable():
    from tools.sound_sim.s12.acoustic_identity_v015.stage_ah.supra_package import _promotion_metadata

    assert _promotion_metadata("R3_PUBLIC_RECORDINGS_UNVERIFIED_UNSYNCHRONIZED") == {
        "promotable": False,
