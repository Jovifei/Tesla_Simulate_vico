...
def hellcat_search_parameters() -> list[SearchParameter]:
    ...
    return [
        ...
        SearchParameter("crank_inertia", 0.34, 0.10, lambda c, v: _set_parameter(c, "crank_inertia", v), ("flux", "dynamic_range"), (), unit="kg_m2"),
        SearchParameter("idle_governor", 0.22, 0.07, lambda c, v: _set_parameter(c, "idle_governor", v), ("flux", "dynamic_range"), (), unit="normalized_torque"),
        ...
        SearchParameter("waveguide_reflection", 1.0, 0.25, lambda c, v: c.setdefault("exhaust_waveguide", {}).update({"reflection_mode": _fixed("open" if v >= 1.0 else "damped", "label", "stage x search reflection mode")}), ("band_120_400", "dynamic_range"), (), unit="mode_scale", note="open vs damped junction reflection"),
        ...
        SearchParameter("timbre_map_order_weights", 1.0, 0.30, lambda c, v: c.setdefault("timbre_mixes", {}).update({"order_weights": _fixed([v, v, 1.0, 1.0], "vector", "stage x order weights")}), ("sharpness", "tonality"), (), unit="vector_scale"),
        SearchParameter("blower_sideband_mix", 1.0, 0.30, lambda c, v: c.setdefault("timbre_mixes", {}).update({"sideband_mix": _fixed(v, "gain", "stage x sideband mix")}), ("tonality", "sharpness"), ("low_band_share",), unit="gain"),
        SearchParameter("blower_broadband_mix", 1.0, 0.30, lambda c, v: c.setdefault("timbre_mixes", {}).update({"broadband_mix": _fixed(v, "gain", "stage x broadband mix")}), ("roughness", "flux"), ("tonality",), unit="gain"),
        SearchParameter("blower_casing_mix", 1.0, 0.30, lambda c, v: c.setdefault("timbre_mixes", {}).update({"casing_mix": _fixed(v, "gain", "stage x casing mix")}), ("tonality",), ("low_band_share",), unit="gain"),
        SearchParameter("intake_mix", 0.18, 0.06, lambda c, v: _set_parameter(c, "intake_model", v), ("mid_band_share", "flux"), (), unit="normalized_gain"),
        SearchParameter("boost_attack", 0.0, 0.08, lambda c, v: c.setdefault("timbre_mixes", {}).update({"boost_attack_s": _fixed(v, "s", "stage x boost attack")}), ("flux", "dynamic_range"), (), unit="s"),
        SearchParameter("boost_release", 0.0, 0.20, lambda c, v: c.setdefault("timbre_mixes", {}).update({"boost_release_s": _fixed(v, "s", "stage x boost release")}), ("flux",), (), unit="s"),
        SearchParameter("bypass_threshold", 0.20, 0.08, lambda c, v: c.setdefault("timbre_mixes", {}).update({"bypass_threshold": _fixed(v, "throttle", "stage x bypass threshold")}), ("tonality", "sharpness"), (), unit="throttle"),
        SearchParameter("afterfire_reservoir_rate", 0.72, 0.20, lambda c, v: c["afterfire"].update({"fuel_reservoir_rate": _fixed(v, "normalized", "stage x reservoir rate")}), ("transient_density",), (), unit="normalized"),
        SearchParameter("afterfire_ignition_delay", 0.004, 0.0015, lambda c, v: _set_parameter(c, "afterfire.ignition_delay_s", v), ("transient_density",), (), unit="s"),
        SearchParameter("afterfire_location_mix", 0.0, 1.0, lambda c, v: _set_parameter(c, "afterfire.event_location", "bank_collector" if v >= 1.0 else "primary"), ("transient_density", "crest"), (), unit="mode", note="primary vs bank collector routing"),
        SearchParameter("afterfire_energy", 0.06, 0.02, lambda c, v: _set_parameter(c, "afterfire.gain", v), ("transient_density", "crest"), (), unit="normalized_gain"),
        SearchParameter("monitor_attack", 0.12, 0.05, lambda c, v: c.setdefault("monitor_policy", {}).update({"attack_s": _fixed(v, "s", "stage x monitor attack")}), ("dynamic_range",), (), unit="s"),
        SearchParameter("monitor_release", 1.20, 0.40, lambda c, v: c.setdefault("monitor_policy", {}).update({"release_s": _fixed(v, "s", "stage x monitor release")}), ("dynamic_range", "crest"), (), unit="s"),
        SearchParameter("monitor_max_makeup", 9.0, 3.0, lambda c, v: c.setdefault("monitor_policy", {}).update({"max_makeup_db": _fixed(v, "dB", "stage x monitor makeup")}), ("dynamic_range", "crest"), ("low_band_share",), unit="dB"),
    ]
...
def _render_config_pcm(config: dict[str, Any], architecture: str, traces: list[Any]) -> list[np.ndarray]:
    ...
    for trace in traces:
        ...
        blocks.append(result.post_ptr_raw if result.post_ptr_raw is not None else result.raw_pcm)
    return blocks
...
def run_parameter_reachability(
    ...
    *,
    ...
    baseline_pcm = _render_config_pcm(base_config, architecture, traces)
    baseline_metrics = _pcm_metrics(baseline_pcm)
    baseline_bytes = b"".join(pcm.tobytes() for pcm in baseline_pcm)
    results = []
    for item in parameters:
        record: dict[str, Any] = {"parameter": item.name, "baseline": item.baseline, "delta": item.delta, "unit": item.unit, "target_metrics": list(item.target_metrics), "note": item.note}
        ...
        for direction, sign in (("minus", -1.0), ("plus", 1.0)):
            ...
            try:
                pcm_blocks = _render_config_pcm(config, architecture, traces)
