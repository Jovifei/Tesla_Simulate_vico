"""Strict Stage-H Hellcat candidate contract."""
...
BASE_COMMIT = "60bca7cccac91c520a12c0b058f3f70d56dcf4b8"
SCHEMA_VERSION = "s12-stage-h-hellcat-candidate-profile-1"
...
SOURCE_KEYS = {
    "blower_gain_scale", "blower_boost_mix", "boost_attack_s", "boost_release_s", "lobe_family_mix",
    "upper_family_tilt_db", "sideband_depth", "bypass_release_gain",
}
COMMON_KEYS = {"idle": set(), "afterfire": {"gain_scale"}, "shift": {"impact_scale", "recovery_scale"}}
REQUIRED_STATES = ["idle", "acceleration", "afterfire"]


@dataclass(frozen=True)
class StageHCandidateProfile:
    ...
    def with_parameter(self, section: str, name: str, value: float) -> "StageHCandidateProfile":
        ...
        if not isinstance(entry, Mapping) or "value" not in entry:
            raise ValueError(f"unknown Stage-H parameter: {section}.{name}")
        entry["value"] = float(value)
        _validate_payload(payload)
        return StageHCandidateProfile(payload, self.path)


def load_stage_h_candidate(path: str | Path) -> StageHCandidateProfile:
    ...
    if reference_sha256(reference_path) != payload["reference_target"]["sha256"]:
        raise ValueError("reference target SHA-256 does not match Stage-H contract")
    return StageHCandidateProfile(payload, candidate_path)


def _validate_payload(payload: Any) -> None:
    if not isinstance(payload, Mapping) or set(payload) != TOP_LEVEL:
        raise ValueError("Stage-H candidate top-level keys mismatch")
    if payload["schema_version"] != SCHEMA_VERSION:
        raise ValueError("unsupported Stage-H schema_version")
    if payload["vehicle_id"] != "hellcat":
        raise ValueError("Stage-H first-pass candidate only supports hellcat")
    if payload["base_commit"] != BASE_COMMIT:
        raise ValueError("candidate base_commit does not match Stage-H baseline")
    if payload.get("status") != "Candidate":
        raise ValueError("Stage-H status must be Candidate")
    for name in ("candidate_id", "parent_candidate_id", "hypothesis", "canonical_trace_version"):
    ...
    if not isinstance(reference["sha256"], str) or len(reference["sha256"]) != 64 or any(c not in "0123456789abcdef" for c in reference["sha256"].lower()):
        raise ValueError("reference target SHA-256 must be hexadecimal")
    ...
    for section in ("source", "idle", "afterfire", "shift"):
        ...
        if unknown:
            raise ValueError(f"unknown Stage-H {section} override: {sorted(unknown)}")
        for name, value in values.items():
            _validate_parameter(f"{section}.{name}", value)
    _validate_loudness(payload.get("loudness"))
    locked = payload.get("locked_layers")
    if not isinstance(locked, Mapping) or any(locked.get(name, {}).get("unchanged") is not True for name in ("low_frequency_body", "rumble", "pre_ptr_eq", "frozen_ptr")):
    ...
    if not isinstance(provenance, Mapping) or provenance.get("source_level") != "C" or provenance.get("source") != "synthetic" or provenance.get("calibration") != "uncalibrated" or provenance.get("claim") != "not OEM reproduction":
        raise ValueError("Stage-H provenance must remain C/synthetic/uncalibrated/not OEM")
...
def _validate_loudness(loudness: Any) -> None:
    ...
    if not isinstance(loudness, Mapping) or set(loudness) != required:
        raise ValueError("Stage-H loudness policy is incomplete")
...
__all__ = ("ANCHOR_IDS", "BASE_COMMIT", "SCHEMA_VERSION", "StageHCandidateProfile", "load_stage_h_candidate", "reference_sha256")
