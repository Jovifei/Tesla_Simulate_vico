...
from audio_parameter_package.package import (
    build_audio_parameter_package,
    validate_audio_parameter_package,
)  # noqa: E402
from engine_product_excitation import (
    build_product_demo_states,
    generate_product_excitation,
    project_order_amplitude,
)  # noqa: E402
...
class EngineOperatingPointLibraryTests(unittest.TestCase):
    def test_library_has_documented_grid_provenance_and_canonical_hash(self):
        ...
        self.assertTrue(
            all(
                entry["source_level"] == "C" and entry["source"] == "synthetic"
                for entry in library.provenance_entries
            )
        )
...
class ProductRendererAndPackageTests(unittest.TestCase):
    def test_renderer_writes_portable_48khz_24bit_stereo_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertEqual(
                metadata["post_ptr_processing_contract"],
                "output_format_only_no_order_eq_limiter_or_synthesis",
            )
            self.assertIn("dc_removal", metadata["processing"])
            self.assertTrue(metadata["synthetic"])
            self.assertEqual(
                json.loads((folder / "fixture.json").read_text(encoding="utf-8")), metadata
            )
    ...
    def test_renderer_resamples_uncontracted_uniform_input_to_48khz(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            metadata = render_product_wav(
                trace,
                pathlib.Path(root) / "resampled.wav",
                pathlib.Path(root) / "resampled.json",
                renderer_profile_from_library(library),
            )
    ...
    def test_audio_parameter_package_is_versioned_json_with_a_hash(self):
        library = load_operating_point_library()
        package = build_audio_parameter_package(
    ...
    def test_audio_parameter_package_pins_frozen_radiation_content(self):
        ...
        self.assertEqual(
            package["ptr_profile"]["radiation_package_sha256"], EXPECTED_RADIATION_PACKAGE_SHA256
        )
        self.assertEqual(
            package["ptr_profile"]["radiation_source_commit"],
...
class ProductVerticalSliceTests(unittest.TestCase):
    def test_product_excitation_maps_rpm_load_and_acceleration_before_ptr(self):
        ...
        quiet = replace(
            states["cruise"],
            case_id="quiet",
            acceleration=tuple(0.0 for _ in states["cruise"].acceleration),
        )
        active = replace(
            states["cruise"],
            case_id="active",
            acceleration=tuple(6.0 for _ in states["cruise"].acceleration),
        )
        self.assertNotEqual(
            generate_product_excitation(quiet, library).pressure_pa,
            generate_product_excitation(active, library).pressure_pa,
        )
    ...
    def test_load_changes_pre_ptr_harmonic_balance(self):
        ...
        low = replace(
            state,
            case_id="low",
            load=tuple(0.05 for _ in state.load),
            throttle=tuple(0.05 for _ in state.throttle),
        )
        high = replace(
            state,
            case_id="high",
            load=tuple(1.0 for _ in state.load),
            throttle=tuple(1.0 for _ in state.throttle),
        )
        low_trace = generate_product_excitation(low, library)
        high_trace = generate_product_excitation(high, library)
        self.assertGreater(
            project_order_amplitude(high_trace, high, 3.0)
            / project_order_amplitude(high_trace, high, 1.0),
            project_order_amplitude(low_trace, low, 3.0)
            / project_order_amplitude(low_trace, low, 1.0),
        )
    ...
    def test_v05_demo_writes_portable_deterministic_product_bundle(self):
        ...
        with tempfile.TemporaryDirectory() as first, tempfile.TemporaryDirectory() as second:
            ...
            required = {
                ...
                "lift.wav",
                "vehicle_state.json",
                "audio_parameter_package.json",
                "manifest.json",
                "SHA256.txt",
            }
            self.assertTrue(required <= {path.name for path in root.iterdir()})
            package = json.loads(
                (root / "audio_parameter_package.json").read_text(encoding="utf-8")
            )
            validate_audio_parameter_package(package)
