(function () {
  ...
  const renderFeedback = (pair) => {
    ...
    return `<div class="card feedback" data-feedback-scope="vehicle"><h2>${esc(vehicleId)} · 车型汇总反馈</h2><p class="load-status">当前车型只提交一份评分；先听当前窗口，再确认本车型听审完成。</p><div class="feedback-grid"><label>软件诊断是否符合听感<select data-feedback="software_agreement"><option value="">请选择</option><option ${f.software_agreement === "符合" ? "selected" : ""}>符合</option><option ${f.software_agreement === "部分符合" ? "selected" : ""}>部分符合</option><option ${f.software_agreement === "不符合" ? "selected" : ""}>不符合</option><option ${f.software_agreement === "无法判断" ? "selected" : ""}>无法判断</option></select></label><label>整体车型身份（0–100）<input data-feedback="identity" type="number" min="0" max="100" value="${esc(f.identity)}"></label><label>整体真实感（0–100）<input data-feedback="realism" type="number" min="0" max="100" value="${esc(f.realism)}"></label><label>更偏好<select data-feedback="preference"><option value="">请选择</option><option ${f.preference === "参考" ? "selected" : ""}>参考</option><option ${f.preference === "候选" ? "selected" : ""}>候选</option><option ${f.preference === "无明显偏好" ? "selected" : ""}>无明显偏好</option></select></label></div><label class="problem-label">最明显问题（点击标签，可多选）<span class="problem-chips">${problems}</span></label><label>自由备注<textarea data-feedback="notes">${esc(f.notes)}</textarea></label><label class="review-check"><input data-feedback="review_ready" type="checkbox" ${f.review_ready ? "checked" : ""}> 我已听完本车型当前窗口对比，确认提交这份车型评分</label><div class="actions"><button class="primary" id="export-feedback" type="button">提交全部车型反馈</button><span id="submit-status" class="submit-status">已完成车型 ${completed}/${vehicleIds().length}；需要三辆车都确认后提交。</span></div></div>`;
  };
  function collectCurrentFeedback(pair) {
    ...
    state.feedback[pair.vehicle_id] = { software_agreement: value("software_agreement"), identity: value("identity"), realism: value("realism"), problems, preference: value("preference"), notes: value("notes"), review_ready: Boolean(root.querySelector("[data-feedback='review_ready']")?.checked) };
  }
  function feedbackComplete(vehicleId) { const f = feedbackFor(vehicleId); return Boolean(f.review_ready) && [f.software_agreement, f.identity, f.realism, f.preference].every(Boolean) && Number(f.identity) >= 0 && Number(f.identity) <= 100 && Number(f.realism) >= 0 && Number(f.realism) <= 100; }
