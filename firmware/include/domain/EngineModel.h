...
class EngineModel {
 public:
  explicit EngineModel(EngineModelConfig config = {}) : config_(config), rpm_(config.idle_rpm) {}
  ...
  VehicleState update(VehicleState input) {
    ...
    rpm_ = (rpm_ * (1.0f - config_.smoothing_alpha)) + (target * config_.smoothing_alpha);

    input.throttle = throttle;
    input.virtual_rpm = std::max(config_.idle_rpm, std::min(rpm_, config_.max_rpm));
  ...
  float targetRpm(float speed_kph, float throttle) const {
    ...
    const float target = config_.idle_rpm + (speed * config_.speed_rpm_gain) +
                         (load * config_.throttle_rpm_gain);
    return std::max(config_.idle_rpm, std::min(target, config_.max_rpm));
  }
