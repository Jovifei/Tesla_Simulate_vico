...
- Fresh evidence: Stage-N post-binding focused tests `9 passed`; eight generated MAT inputs have 8/8 manifest SHA matches and the expected PCM24/RPM/state schemas; full `tools/sound_sim/s12/tests` `343 passed, 114 subtests` in `379.69s`; historical Stage-N publisher determinism and artifact-manifest verification remain valid.
- Boundary: MATLAB adapter execution awaits exactly one manually opened safe Desktop with an existing-session control path. The eight hash-bound project inputs are prepared separately from execution evidence; a real external reference must still carry lawful raw waveform provenance plus matching RPM/state metadata. After a SHA/file-ID-bound Jovi CSV or webMUSHRA result, use a separate sound-fix branch; do not change vehicle sources here.
