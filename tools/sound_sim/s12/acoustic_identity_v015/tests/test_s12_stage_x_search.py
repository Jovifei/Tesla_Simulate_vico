"""Stage X/Y tests: parameter domains, evidence gates, and R1 fixture."""
...
def test_sobol_and_refine_stay_in_bounds_and_categories_are_discrete() -> None:
    parameters = hellcat_search_parameters()
    coarse = sobol_overrides(parameters, 16, seed=11)
    ...
    for row in coarse:
        ...
        assert row["waveguide_reflection"] in {0.0, 1.0}
        assert row["afterfire_location_mix"] in {0.0, 1.0}
        assert row["attack_mix_120_400"] >= 0.0
        assert row["boost_attack"] >= 0.0
        assert row["boost_release"] >= 0.0
    assert {row["waveguide_reflection"] for row in coarse} == {0.0, 1.0}
    assert {row["afterfire_location_mix"] for row in coarse} == {0.0, 1.0}

    center = {item.name: item.baseline for item in parameters}
    refined = refine_overrides(parameters, center, 8, seed=12)
    for row in refined:
        for item in parameters:
            assert item.baseline - item.delta - 1e-9 <= row[item.name] <= item.baseline + item.delta + 1e-9
        assert row["waveguide_reflection"] in {0.0, 1.0}
        assert row["afterfire_location_mix"] in {0.0, 1.0}
    assert by_name["waveguide_reflection"].baseline == 1.0


def test_engineering_gate_eligible_on_strong_evidence_record() -> None:
    ...
    assert gate["failed_hard_gates"] == []
    assert gate["eligibility"]["selection_eligible"] is True
    assert gate["status"] == "R2_ENGINEERING_PRESELECTION"


def test_engineering_gate_fails_closed_when_evidence_is_missing() -> None:
    gate = eg.evaluate_engineering_preselection(
        {
            "finite": True,
            "clipping_samples": 0,
            "click_ok": True,
            "comparison": {
                "improvement_fraction": 0.30,
                "dimension_median_relative_error": {
                    "low_frequency_body": -0.2,
                    "120_400_pressure_attack": -0.2,
                    "mid_band_congestion": -0.2,
                    "synthetic_artifact": -0.1,
                },
            },
        },
        architecture="P3",
    ...
    assert gate["hard_gates_passed"] is False
    assert "parent_sha_different" in gate["failed_hard_gates"]
    assert "post_ptr_exists" in gate["failed_hard_gates"]
    assert "parameter_consumed" in gate["failed_hard_gates"]
    assert gate["eligibility"]["selection_eligible"] is False


def test_engineering_gate_uses_independent_sources_not_scenario_windows() -> None:
    gate = eg.evaluate_engineering_preselection(
        _good_record(independent_reference_count=1),
        architecture="P3",
    ...
    assert gate["hard_gates_passed"] is True
    assert gate["soft_gates"]["independent_reference_count_at_least_2"] is False
...
def test_engineering_gate_fail_closed_paths() -> None:
    weak = _good_record(
        comparison={
            ...
            "dimension_median_relative_error": {
                "low_frequency_body": 0.0,
                "120_400_pressure_attack": 0.0,
                "mid_band_congestion": 0.0,
                "synthetic_artifact": 0.0,
            },
        }
    )
    gate = eg.evaluate_engineering_preselection(
    ...
    gate2 = eg.evaluate_engineering_preselection(
        clipped,
        architecture="P3",
        valid_reference_count=4,
        independent_reference_count=4,
...
def test_r1_fixture_pipeline_is_ready_but_fail_closed(tmp_path) -> None:
    ...
    order = fgf.export_matlab_order_input(
        [_case_from_dict(case) for case in receipt["cases"]],
        tmp_path / "order_input.json",
    )
    assert order["order_metric_status"] == "QUALIFIED_WITH_SYNCHRONIZED_RPM"
    result = fgf.evaluate_formal_selection(
        receipt,
        {"P3": 0.42, "P2H": 0.10},
...
def test_review_package_validator_fail_closed(tmp_path) -> None:
    ...
    manifest_dir.mkdir()
    (manifest_dir / "package_manifest.json").write_text(
        json.dumps({
            "schema": "s12.stage_x.review_package.v1",
            "boundary": "",
            "files": {"a.wav": "0" * 64},
            "scenarios": {},
        }),
...
def test_loudness_match_preserves_spectrum() -> None:
    ...
    assert abs(
        float(np.sqrt(np.mean(np.square(matched.mean(axis=1)))))
        - float(np.sqrt(np.mean(np.square(reference.mean(axis=1)))))
    ) < 1e-9
