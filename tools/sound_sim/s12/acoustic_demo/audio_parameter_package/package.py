...
def validate_audio_parameter_package(package: dict) -> None:
    required = {"version", "engine_id", "rpm_range", "load_range", "excitation_profile", "ptr_profile", "renderer_profile", "hash", "source_commit", "synthetic", "provenance"}
    ...
    if package["synthetic"] is not True or not isinstance(package["engine_id"], str) or not package["engine_id"]:
    ...
    if not all(isinstance(value, list) and len(value) == 2 and all(_finite_number(item) for item in value) and value[0] <= value[1] for value in (package["rpm_range"], package["load_range"])):
    ...
    if set(excitation) != {"interpolation", "operating_point_library_hash", "synthetic"} or excitation["interpolation"] != "bilinear" or excitation["synthetic"] is not True or not isinstance(excitation["operating_point_library_hash"], str) or not re.fullmatch(r"[0-9a-f]{64}", excitation["operating_point_library_hash"]):
        raise ValueError("AudioParameterPackage excitation profile is invalid")
    if set(ptr) != {"configuration", "frozen_math", "radiation_package_sha256", "radiation_source_commit"} or not isinstance(ptr["configuration"], str) or not ptr["configuration"] or ptr["frozen_math"] is not True or not isinstance(ptr["radiation_package_sha256"], str) or not re.fullmatch(r"[0-9a-f]{64}", ptr["radiation_package_sha256"]) or not isinstance(ptr["radiation_source_commit"], str) or not re.fullmatch(r"[0-9a-f]{40}", ptr["radiation_source_commit"]):
        raise ValueError("AudioParameterPackage PTR profile is invalid")
    if set(renderer) != {"sample_rate_hz", "gain_db", "edge_fade_s"} or not isinstance(renderer["sample_rate_hz"], int) or isinstance(renderer["sample_rate_hz"], bool) or renderer["sample_rate_hz"] <= 0 or not _finite_number(renderer["gain_db"]) or not _finite_number(renderer["edge_fade_s"]) or renderer["edge_fade_s"] < 0.0:
        raise ValueError("AudioParameterPackage renderer profile is invalid")
    if set(provenance) != {"source_level", "source", "description"} or provenance["source_level"] != "C" or provenance["source"] != "synthetic" or not isinstance(provenance["description"], str) or not provenance["description"]:
