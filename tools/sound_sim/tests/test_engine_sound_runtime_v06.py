...
class RuntimeVehicleStateTests(unittest.TestCase):
    def test_drive_cycle_is_100hz_continuous_and_contains_required_modes(self):
        ...
        self.assertTrue(
            all(800.0 <= state.rpm <= 6000.0 and 0.0 <= state.load <= 1.0 for state in states)
        )
        self.assertLess(
            max(abs(current.rpm - previous.rpm) for previous, current in zip(states, states[1:])),
            20.0,
        )
        self.assertLess(
            max(abs(current.load - previous.load) for previous, current in zip(states, states[1:])),
            0.02,
        )
        ...
        self.assertTrue(
            {
                RuntimeMode.IDLE,
                RuntimeMode.CRUISE,
                RuntimeMode.ACCELERATION,
                RuntimeMode.DECELERATION,
                RuntimeMode.HIGH_LOAD,
            }
            <= observed
        )
    ...
    def test_state_machine_smooths_mode_changes(self):
        ...
        for index in range(2, 12):
            transition = state_machine.update(
                VehicleState(index / 100.0, 1600.0, 8.0, 2.0, 0.6, 0.6)
            )
...
class RuntimeAudioTests(unittest.TestCase):
    ...
    def test_audio_callback_keeps_phase_across_20ms_pcm_blocks(self):
        ...
        self.assertEqual(
            (first.sample_rate_hz, first.channels, first.bits_per_sample), (48000, 2, 24)
        )
    ...
    def test_load_and_acceleration_change_runtime_pcm_before_ptr(self):
        ...
        self.assertNotEqual(
            EngineSoundRuntime().audio_callback(steady).pcm_s24le_stereo,
            EngineSoundRuntime().audio_callback(loaded).pcm_s24le_stereo,
        )
        self.assertNotEqual(
            EngineSoundRuntime().audio_callback(steady).pcm_s24le_stereo,
            EngineSoundRuntime().audio_callback(accelerating).pcm_s24le_stereo,
        )
        ...
        for index in range(101):
            ...
            if previous is not None:
                maximum_boundary_delta = max(
                    ...
                    abs(frame.normalized_samples[0] - previous.normalized_samples[-1]),
                )
            previous = frame
    ...
    def test_stateful_ptr_adapter_matches_the_frozen_batch_network(self):
        ...
        trace = PressureTrace.uniform(
            "runtime-ptr", samples, 48000, 40.0, "engine_exhaust_port", ("synthetic",)
        )
    ...
    @unittest.skipUnless(
        sys.platform == "win32", "Windows waveOut is a Windows-only PC audio adapter"
    )
...
class RuntimeProductContractTests(unittest.TestCase):
    ...
    def test_app_nan_payload_reaches_runtime_fallback(self):
        state = parse_app_vehicle_state(
            {"speed": math.nan, "acceleration": 0.5, "timestamp": 123.0}
        )
    ...
    def test_audio_parameter_package_v02_contains_runtime_controls(self):
        ...
        self.assertEqual(
            set(package["runtime_profile"]),
            {"rpm_map", "load_map", "transition_curve", "renderer_config", "pcm"},
        )
        self.assertEqual(
            package["runtime_profile"]["pcm"],
            {"sample_rate_hz": 48000, "block_samples": 960, "channels": 2, "bits_per_sample": 24},
        )
    ...
    def test_runtime_demo_is_deterministic_and_writes_only_a_report(self):
        with (
            ...
            self.assertFalse(
                any(path.suffix.lower() == ".wav" for path in pathlib.Path(left_root).rglob("*"))
            )
            ...
            markdown = left.report_path.with_name(
                "S12_Runtime_Engine_Sound_v06_Report.md"
