...
def run_v05_demo(output_root: Path, source_commit: str | None = None) -> V05Result:
    ...
    for name, state in states.items():
        ...
        metadata = render_product_wav(ptr, demo / f"{name}.wav", demo / "metadata" / f"{name}.json", renderer_profile)
        metadata.update({
    ...
    _write_json(demo / "vehicle_state.json", {"schema": "s12.vehicle_state.v05", "synthetic": True, "cases": {name: {field: list(getattr(state, field)) for field in ("timestamp", "rpm", "speed", "acceleration", "load", "throttle")} for name, state in states.items()}})
    _write_json(demo / "audio_parameter_package.json", parameter_package)
    _write_json(demo / "sound_analysis.json", {"schema": "s12.sound_analysis.v05", "generator_version": GENERATOR_VERSION, "cases": analysis})
    ...
    manifest_path = _write_json(demo / "manifest.json", {
        "architecture": "operating_point_to_excitation_to_ptr_to_radiation_to_renderer",
