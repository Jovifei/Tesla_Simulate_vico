...
_VEHICLE_ALIASES = {"hellcat":{"hellcat","hellcat_v1"},"ferrari_458":{"ferrari_458","ferrari_458_v1"},"lfa":{"lfa","lfa_v1"},"gtr_r35":{"gtr_r35","gtr_r35_v1"}}
...
def family_parameters(config: Mapping[str,Any], family: str) -> list[SearchParameter]:
    paths={"body":["combustion_event.event_energy","combustion_event.rise_time_s","combustion_event.decay_time_s","cycle_variation"],"path":["collector_loss","intake_model","collector_length_m"],"induction":["forced_induction.gain","forced_induction.ratio","primary_spool_tau","secondary_spool_tau","blow_off_gain","blow_off_decay"],"afterfire":["afterfire.gain","afterfire.cooldown_s","afterfire.ignition_delay_s"]}
...
def apply_overrides(config: Mapping[str,Any], overrides: Mapping[str,float], parameters: list[SearchParameter]) -> dict[str,Any]:
    ...
    if unknown: raise ValueError(f"unknown Stage-AE overrides: {sorted(unknown)}")
...
def _read_reference(case: Mapping[str,Any], sample_rate: int=48000) -> np.ndarray:
    ...
    if data.ndim!=2 or data.shape[1] not in {1,2}: raise ValueError("reference must be mono/stereo")
    ...
    if end<=start: raise ValueError("reference segment is empty")
    segment=data[start:end]
    if not np.all(np.isfinite(segment)): raise ValueError("reference contains non-finite samples")
    return segment
...
def validate_caseset_identity(payload: Mapping[str,Any], vehicle_key: str) -> None:
    if vehicle_key not in _VEHICLE_ALIASES: raise ValueError(vehicle_key)
    declared=str(payload.get("vehicle_id") or "")
    if declared and declared not in _VEHICLE_ALIASES[vehicle_key]:
        raise ValueError(f"caseset vehicle_id {declared!r} does not match {vehicle_key!r}")


def load_caseset(path: str|Path, vehicle_key: str) -> tuple[dict[str,np.ndarray],str]:
    payload=json.loads(Path(path).read_text(encoding="utf-8")); validate_caseset_identity(payload,vehicle_key)
    effective=str(payload.get("reference_evidence_level") or "UNKNOWN"); refs={}
    for case in payload.get("cases",[]):
        case_vehicle=str(case.get("vehicle_id") or "")
        if case_vehicle and case_vehicle not in _VEHICLE_ALIASES[vehicle_key]: raise ValueError(f"case vehicle mismatch: {case_vehicle}")
...
def evaluate_config(vehicle_key: str, config: Mapping[str,Any], references: Mapping[str,np.ndarray], seed: int=20260905, duration_s: float=2.0) -> dict[str,Any]:
    ...
    for scene,ref in references.items():
        ...
        if n<=0: raise ValueError(f"empty comparison for {scene}")
        ref_trim=ref[:n]; cand_trim=cand[:n]; finite=bool(np.all(np.isfinite(cand_trim))); click=block_boundary_click_metrics(cand_trim,960); all_finite &= finite; all_clicks &= bool(click["passed"])
        comp=compare_case(ref_trim,cand_trim,cand_trim,48000,candidate_id=f"{vehicle_key}_stage_ae"); dist=_distance(comp)
        scene_rows[scene]={"distance":dist,"peak":float(np.max(np.abs(cand_trim))),"click_passed":bool(click["passed"])}
...
def run_family_fit(vehicle_key: str, caseset_path: str|Path, family: str, output_dir: str|Path, samples: int=16, seed: int=20260905, base_config_path: str|Path|None=None) -> dict[str,Any]:
    ...
    profile=VEHICLES[vehicle_key]; config=json.loads(Path(base_config_path).read_text(encoding="utf-8")) if base_config_path else load_config(profile.config_id); config=validate_config(config)
    refs,evidence=load_caseset(caseset_path,vehicle_key); params=family_parameters(config,family); baseline=evaluate_config(vehicle_key,config,refs,seed); best_cfg=copy.deepcopy(config); best_eval=baseline; best_overrides={}
    if params:
        ...
        for point in points:
            overrides={p.name:float(p.minimum+u*(p.maximum-p.minimum)) for p,u in zip(params,point)}; trial=apply_overrides(config,overrides,params); result=evaluate_config(vehicle_key,trial,refs,seed)
            if result["passed_gates"] and np.isfinite(result["median_distance"]) and result["median_distance"] < best_eval["median_distance"]: best_cfg,best_eval,best_overrides=trial,result,overrides
    out=Path(output_dir); out.mkdir(parents=True,exist_ok=True); final_path=out/"final_r3_diagnostic_fit.json"; final_path.write_text(json.dumps(best_cfg,indent=2,ensure_ascii=False),encoding="utf-8")
    ...
    (out/"family_fit_receipt.json").write_text(json.dumps(receipt,indent=2,ensure_ascii=False),encoding="utf-8"); return receipt
