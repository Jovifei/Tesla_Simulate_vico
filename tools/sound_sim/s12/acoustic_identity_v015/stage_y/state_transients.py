...
class StateTransientMixer:
    ...
    def __init__(self, sample_rate_hz: int = 48000) -> None:
        ...
        self._tail_length = int(round(0.080 * self.sample_rate_hz))
        self._tip_in = False
    ...
    def render_block(self, n: int, throttle: float, rpm: float, boost: float, dt: float) -> tuple[np.ndarray, dict[str, float | int]]:
        ...
        if rpm_drop > 800.0 and duration < 0.080:
            self._trigger("shift", 90.0, 0.120, 0.055)
        if current_boost < self._last_boost - 0.05 and current_throttle < 0.20:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {"schema_version": "s12.stage_y.state_transients.v1", "sample_rate_hz": self.sample_rate_hz, "tail_length": self._tail_length, "tip_in": self._tip_in, "last_throttle": self._last_throttle, "last_rpm": self._last_rpm, "last_boost": self._last_boost, "counts": dict(self._counts), "audio_tails": {name: value.copy() for name, value in self._audio_tails.items()}, "mix_tails": {name: value.copy() for name, value in self._mix_tails.items()}}

    def restore(self, payload: Mapping[str, Any]) -> None:
        state = self._validate_snapshot(payload)
        self._tip_in, self._last_throttle, self._last_rpm, self._last_boost = state["tip_in"], state["last_throttle"], state["last_rpm"], state["last_boost"]
    ...
    def _validate_snapshot(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        required = {"schema_version", "sample_rate_hz", "tail_length", "tip_in", "last_throttle", "last_rpm", "last_boost", "counts", "audio_tails", "mix_tails"}
        ...
        if type(payload["tip_in"]) is not bool:
            raise ValueError("transient tip-in state must be boolean")
        state = {"tip_in": payload["tip_in"], "last_throttle": self._finite(payload["last_throttle"], "transient last throttle"), "last_rpm": self._finite(payload["last_rpm"], "transient last rpm"), "last_boost": self._finite(payload["last_boost"], "transient last boost")}
