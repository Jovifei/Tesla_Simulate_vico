...
- [x] AA1：逐层审计 vehicle_state→monitor 的能量账本，定位 RMS/dynamic range 丢失层，建立 `energy_budget_trace.json` 与 root-cause 报告。
...
- [x] Merge the verified Stage AG-R1 source plus strict-route documentation into `main`; current SHA must be read with `git rev-parse origin/main`, and PR #16 is closed.
