...
class EngineSoundV04Tests(unittest.TestCase):
    def test_all_v04_contract_values_are_source_classified(self):
        for name in (
            "engine_state_schema.json",
            "order_profile.json",
            "engine_excitation_parameters.json",
        ):
            package = load_json_package(DEMO_ROOT / name)
            entries = (
                package["fields"] if name == "engine_state_schema.json" else package["parameters"]
            )
    ...
    def test_state_contract_consumes_speed_and_acceleration(self):
        ...
        fast = EngineStateInput(
            ...
            state.rpm,
            state.speed,
            tuple(4.0 for _ in state.acceleration),
            state.load,
            state.throttle,
        )
        slow = EngineStateInput(
            ...
            state.timestamp,
            state.rpm,
            tuple(0.0 for _ in state.speed),
            tuple(0.0 for _ in state.acceleration),
            state.load,
            state.throttle,
        )
    ...
    def test_speed_only_change_propagates_to_ptr_then_audio(self):
        ...
        fast = EngineStateInput(
            ...
            state.timestamp,
            state.rpm,
            tuple(30.0 for _ in state.speed),
            state.acceleration,
            state.load,
            state.throttle,
        )
        slow_ptr, fast_ptr = (
            run_ptr_network(generate_engine_excitation(slow)),
            run_ptr_network(generate_engine_excitation(fast)),
        )
        with tempfile.TemporaryDirectory() as root:
            folder = pathlib.Path(root)
            render_ptr_trace_wav(
                slow_ptr, folder / "slow.wav", folder / "slow.json", 48000, 0.70, 0.004
            )
            render_ptr_trace_wav(
                fast_ptr, folder / "fast.wav", folder / "fast.json", 48000, 0.70, 0.004
            )
            self.assertNotEqual(
                (folder / "slow.wav").read_bytes(), (folder / "fast.wav").read_bytes()
            )

    def test_acceleration_change_propagates_to_ptr_then_audio(self):
        states = build_default_engine_state_cases()
        state = states["cruise"]
        quiet = EngineStateInput(
            "quiet",
            state.timestamp,
            state.rpm,
            state.speed,
        ...
        active = EngineStateInput(
            "active",
            state.timestamp,
            state.rpm,
            state.speed,
            tuple(6.0 for _ in state.acceleration),
            state.load,
            state.throttle,
        )
        quiet_source, active_source = (
            generate_engine_excitation(quiet),
            generate_engine_excitation(active),
        )
        quiet_ptr, active_ptr = run_ptr_network(quiet_source), run_ptr_network(active_source)
        self.assertNotEqual(
            quiet_source.source_identity_sha256, active_source.source_identity_sha256
        )
        ...
        with tempfile.TemporaryDirectory() as root:
            folder = pathlib.Path(root)
            render_ptr_trace_wav(
                quiet_ptr, folder / "quiet.wav", folder / "quiet.json", 48000, 0.70, 0.004
            )
            render_ptr_trace_wav(
                active_ptr, folder / "active.wav", folder / "active.json", 48000, 0.70, 0.004
            )
            self.assertNotEqual(
                (folder / "quiet.wav").read_bytes(), (folder / "active.wav").read_bytes()
            )
    ...
    def test_v04_demo_is_deterministic_and_renderer_is_48khz_24bit_stereo(self):
        ...
        with tempfile.TemporaryDirectory() as first, tempfile.TemporaryDirectory() as second:
            ...
            metadata = json.loads(
                (root / "metadata" / "acceleration.json").read_text(encoding="utf-8")
            )
