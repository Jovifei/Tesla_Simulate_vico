...
def load_ir_asset(spec: IrAssetSpec, target_sample_rate_hz: int = 48000, use: str = "diagnostic") -> np.ndarray:
    ...
    sample_rate, data = wavfile.read(spec.path)
    if data.ndim > 1:
        data = np.mean(data.astype(np.float64), axis=1)
    else:
        data = data.astype(np.float64)
    if np.issubdtype(data.dtype, np.integer):
        raise AssertionError("integer conversion must occur before dtype coercion")
    # wavfile has already been coerced to float64 above; infer original range conservatively.
    peak = float(np.max(np.abs(data))) if data.size else 0.0
    if peak > 2.0:
        data /= 32768.0 if peak <= 32768.0 else peak
