...
目标：把已经绿的 validation hardening 正常进入 main。

完成标准：

- PR 当前 head 与 green CI exact match；
- Track-P / repository checks green；
- no unexpected production PCM tuning；
- normal merge，无 force/rewrite；
- post-merge ancestry 和 smoke receipt。

注意：`tasks/reports/runtime/s12-stage-ac/execution_state.json` 在 PR branch 上仍有异步状态债务（AC6 仍写 IN_PROGRESS），但最新 CI 已实际成功。更新任何 PR 文件都会改变 head，必须重新等待新 CI。
...
2. Close PR #5 / Stage AC without invalidating exact-head CI
