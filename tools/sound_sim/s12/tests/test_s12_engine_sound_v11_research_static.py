...
OBSIDIAN = Path(r"E:\AI_Tools\Obsidian\data\notes-personal\tesla\S12-Engine-Sound-v11")
VEHICLES = (
    "hellcat_2022_stock",
    "gtr_r35_2007_stock",
...
class S12EngineSoundV11ResearchStaticTests(unittest.TestCase):
    def test_manifests_catalog_nonempty_honest_research_references(self) -> None:
        for vehicle_id in VEHICLES:
            ...
            self.assertTrue(any(reference["source_classification"] == "official_manufacturer_configuration"
                                for reference in manifest["references"]))
            self.assertTrue(any(reference["source_classification"] == "public_candidate_sound_reference_discovery"
                                for reference in manifest["references"]))
            for reference in manifest["references"]:
                self.assertEqual(set(reference), REQUIRED_REFERENCE_FIELDS)
                self.assertTrue(reference["source_url"].startswith("https://"))
                self.assertEqual(reference["source_url_sha256"], url_sha256(reference["source_url"]))
                self.assertEqual(reference["vehicle_binding"], manifest["vehicle_identity"])
                self.assertIn(reference["vehicle_binding_state"], {
                    "identity_verified", "identity_pending", "query_scoped_candidate_not_verified"
                })
                self.assertIn(reference["stock_modified_status"], {
                    "manufacturer_identity_only", "manufacturer_identity_pending", "unverified_candidate"
                })
                if reference["source_classification"] == "official_manufacturer_configuration":
                    self.assertIn(
                        (reference["vehicle_binding_state"], reference["stock_modified_status"]),
                        {
                    ...
                    self.assertEqual(reference["vehicle_binding_state"], "query_scoped_candidate_not_verified")
                    self.assertEqual(reference["stock_modified_status"], "unverified_candidate")
                self.assertIn(reference["listening_perspective"], {"not_applicable", "not_available"})
                self.assertEqual(reference["clip_time_metadata"], {
    ...
    def test_schema_and_validator_require_honest_nonempty_manifests(self) -> None:
        ...
        self.assertIn("Reference manifests must catalog at least one honest non-audio source.", validator)
    ...
    def test_vehicle_years_are_structured_and_c63_uses_facelift_source(self) -> None:
        ...
        official = next(reference for reference in c63["references"]
                        if reference["source_classification"] == "official_manufacturer_configuration")
        self.assertEqual(official["source_url"], C63_FACELIFT_SOURCE)
        self.assertEqual(official["source_url_sha256"], url_sha256(C63_FACELIFT_SOURCE))
    ...
    def test_profile_manufacturer_source_matches_official_manifest_source(self) -> None:
        for vehicle_id in VEHICLES:
            ...
            self.assertEqual(profile["provenance"]["source_type"], "manufacturer", vehicle_id)
            self.assertTrue(profile["provenance"]["source_url"].startswith("https://"), vehicle_id)
            manifest = load_manifest(vehicle_id)
            official_urls = {
            ...
            self.assertIn(profile["provenance"]["source_url"], official_urls, vehicle_id)
        validator = (V11 / "common" / "s12_v11_validate_vehicle_package.m").read_text(encoding="utf-8")
        self.assertIn("validateProfileManifestSourceLink", validator)
        self.assertIn("Profile A/manufacturer source_url must equal one official manufacturer manifest source_url.", validator)
        self.assertIn("Canonical profile provenance must be source level A and manufacturer.", validator)
        self.assertNotIn('if string(provenance.source_level) ~= "A" || string(provenance.source_type) ~= "manufacturer"\n    return;', validator)
        schema = json.loads((V11 / "common" / "schemas" / "vehicle_package.schema.json").read_text(encoding="utf-8"))
        self.assertEqual(schema["profile_provenance_contract"], {
            "source_level": "A", "source_type": "manufacturer"
        })

    def test_matlab_provenance_suite_has_bad_year_and_semantic_pair_fixtures(self) -> None:
        suite = (ROOT / "tests" / "test_s12_engine_sound_v11_provenance.m").read_text(encoding="utf-8")
        for test_name in (
...
def load_manifest(vehicle_id: str) -> dict:
    return json.loads((V11 / "vehicles" / vehicle_id / "reference_manifest.json").read_text(encoding="utf-8"))
