...
`Y9_FINAL_QUALIFICATION`: final committed fitted-map reachability is PASS;
...
unchanged. The earlier full S12 receipt is historical and is superseded by the
canonical fixture-hash repair; one new full S12 run is required on the final
map/code HEAD.

Authoritative current proof: `final_qualification/y1_final_fitted_map_reachability_receipt_v2.json`
and `y1_reachability/final_fitted_map_canonical/parameter_reachability.json`. The old
...
- Current phase: `Y9_FINAL_QUALIFICATION` (`IN_PROGRESS`); final fitted-map receipt passed, full S12 is pending after canonical hash repair.
...
Do-not-rerun-long-task: after the one explicitly authorized canonical-hash
qualification completes, do not rerun the final fitted-map selected-16 probe or
