...
本清单用于 S7 之后持续追踪。它不是“全部已经完成”的证明；当前优先级是先完成 BLE / WiFi / IoT / OTA 架构迁移门禁，再做硬件验收和声浪算法。
...
| P0-04 | BLE `ffe8` 配置写入与读回 | 高 | 代码已迁移 | 写入 JSON 后 SD/读回一致 |
...
| ID | 待完成项 | 优先级 | 当前状态 | 验收证据 |
|---|---|---|---|---|
| S7-01 | 统一运行状态镜像 | 高 | 代码已完成 | `ffe5` 包含 WiFi/IoT/OTA/版本/分区/错误 |
| S7-02 | RuntimeConfig WiFi/OTA/IoT 字段 | 高 | 代码已完成 | 旧 JSON 缺字段仍可加载，新字段可保存 |
| S7-03 | NetworkManager WiFi 状态机 | 高 | 代码已完成 | 未配置、连接中、已连接、失败、重连状态可见 |
| S7-04 | IotManager MQTT 上线 | 高 | 代码已完成，待实机 | 上报 device info / vehicle state |
| S7-05 | MQTT 下行 `ota_start` | 高 | 代码已完成，待实机 | accepted/rejected ack，accepted 转 OTA request |
| S7-06 | HTTPS OTA 成功路径 | 高 | 待实机 | 下载、切分区、重启后版本变化 |
| S7-07 | HTTPS OTA 失败路径 | 高 | 待实机 | 坏 URL/坏镜像不损坏当前固件 |
| S7-08 | BLE 写配置不阻塞主循环 | 高 | 代码已完成 | 写 `ffe8` 不在回调内执行 OTA |
| S7-09 | App 25 ms 主循环不阻塞 | 高 | 待实机 | build + 代码审查 + 实机观察 |
...
| MEM-03 | IRAM 1 byte margin 处理 | 高 | 未关闭 | 降低风险、功能分档或形成实机压力测试接受记录 |
| MEM-04 | BLE/WiFi/OTA/WS2812 并发压测 | 高 | 待实机 | OTA 时 BLE 不掉线或有可接受降级策略 |
| MEM-05 | SPI flash ROM driver 实验 | 中 | 已验证但不采纳 | 能减 DIRAM/镜像，不能减 IRAM，且牺牲 auto-suspend |
...
| AUD-03 | MATLAB 或等效仿真 | 高 | 待开始 | 曲线、波形、参数、听感记录 |
...
| DOC-05 | GitHub README 保持真实 | 中 | 持续维护 | 不夸大完成度，无乱码 |
...
4. 处理 MEM-01 到 MEM-05 的 IRAM release gate。
